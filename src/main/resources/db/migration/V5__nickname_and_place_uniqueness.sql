...
-- 결과도 유일하다 — 누군가 이미 "X-7" 같은 이름을 쓰고 있는 경우만 예외다.
--
-- 주의: 그때 "마이그레이션 전체가 롤백된다"는 이전 주석은 **사실이 아니었다**. MySQL은 DDL이
-- 트랜잭션이 아니고 ALTER TABLE이 암묵적 커밋을 유발하므로, 아래 ALTER가 실행되는 시점에
-- 이 UPDATE는 이미 커밋돼 있다. flyway-mysql도 MySQL에 트랜잭션 롤백을 제공하지 않는다.
-- 실패하면 남는 상태는 이렇다: 닉네임 변경은 커밋된 채 남고(충돌하지 않은 사용자 이름도 이미
-- 바뀜), UNIQUE 인덱스는 없고, Flyway 이력에 V5가 failed로 남아 앱이 기동하지 못한다.
-- 아래 SET r.kakao_place_id = NULL도 같은 성질이라 중복 행의 place id가 커밋된 채 사라진다.
--
-- 앞으로의 마이그레이션은 데이터 정리 UPDATE와 제약 추가 DDL을 별도 버전으로 나누고,
-- 제약 추가 전에 사전 검증(SELECT nickname, COUNT(*) ... HAVING COUNT(*) > 1)을 둔다(#74).
--
...
-- 주의: 이 RENAME은 expand/contract 없는 단일 단계 변경이라 **이미지 롤백 경로를 끊는다.**
-- 세 프로파일 모두 ddl-auto=validate이므로 V5 이전 앱은 naver_place_id를 요구하다 기동
-- 검증에서 실패하고 restart 루프에 빠진다. 되돌리려면 컬럼을 손으로 되돌리고
-- flyway_schema_history에서 V5를 지워야 하며 그동안 서비스는 내려가 있다.
-- 앞으로의 컬럼 교체는 (Va) 새 컬럼 추가 + 백필 + 양쪽 쓰기 → (Vb) 안정화 후 옛 컬럼 드롭으로
-- 나눈다. docker-compose.prod.yml의 롤백 절차 주석도 이 사실을 함께 적고 있다(#74).
--
