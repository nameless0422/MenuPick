 ...
 * 여럿이 같이 뽑기.
 *
 * <h2>만들기만 로그인, 나머지는 링크</h2>
 *
 * <p>방을 만드는 것은 자기 메뉴를 꺼내 공유하는 행위라 계정이 필요하다. 반대로 참여는 링크를
 * 받은 사람이면 누구나 할 수 있어야 한다 — 점심 자리에 있는 사람 전원이 가입해 있을 리 없고,
 * 가입을 요구하는 순간 방을 만들 이유가 사라진다. 그래서 {@code /rooms/{code}} 아래 경로는
 * {@code SecurityConfig}에서 permitAll이고, 방 코드가 곧 입장 자격이다.
 *
...
public class PickRoomController {
    ...
    public ResponseEntity<ApiResponse<PickRoomResponse>> get(
        ...
        return ResponseEntity.ok(ApiResponse.ok(pickRoomService.get(code, participant)));
    }
