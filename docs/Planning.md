...
| --- | --- | --- | --- |
| POST | /auth/kakao | 카카오 OAuth 로그인 — 가입은 되지 않는다(연동된 계정이 없으면 401 `SOCIAL_ACCOUNT_NOT_LINKED`) | N |
| POST | /auth/google | 구글 OAuth 로그인 — 위와 같다 | N |
| POST | /auth/{provider}/link | 로그인한 계정에 소셜 계정 연동 — 연동 후 `linkedProviders` 반환 | Y |
| DELETE | /auth/{provider}/link | 연동 해제 — 남는 로그인 수단이 없으면 409 `LAST_LOGIN_METHOD` | Y |
...
| 4 | 백엔드 | social_id + provider 기준으로 기존 사용자 조회 — 없으면 거절한다(가입시키지 않는다) |
...
**소셜은 가입 경로가 아니다**: 카카오는 비즈앱 전환(사업자 정보 등록) 없이는 이메일을 주지 않는다. 그 상태로 소셜 가입을 받으면 `users.email`이 NULL인 계정이 생기고, 그 계정은 비밀번호 재설정도 안내 메일 수신도 불가능한 고아 계정이 된다. 그래서 가입은 이메일/비밀번호 하나로만 받고, 소셜은 **로그인한 상태에서 명시적으로 연동**하는 부가 수단으로만 쓴다(`POST /auth/{provider}/link`). 연동된 적 없는 소셜 계정으로 로그인하면 새 유저를 만들지 않고 401(`SOCIAL_ACCOUNT_NOT_LINKED`)로 거절하며, 프론트는 이 코드를 보고 로그인 화면에서 가입 안내를 띄운다.

**계정 통합 정책 (이메일 기준 자동 통합)**: 처음 보는 (provider, social_id)라도 **제공자가 소유를 검증한 이메일**과 같은 주소로 가입된 유저가 있으면 그 유저에 소셜 연동(auth_providers 행)만 추가하고 로그인시킨다 — 구글은 이 경로로 계속 동작한다. 같은 사람이 카카오·구글로 각각 로그인해도 계정이 하나로 유지되고, `uq_users_email` UNIQUE 제약과도 충돌하지 않는다. 미검증 이메일을 신뢰하면 공격자가 타인 주소를 자기 소셜 계정에 넣어 계정을 탈취할 수 있으므로, 검증되지 않았거나 아예 없으면 통합하지 않고 위와 같이 거절한다.

**연동 거절 규칙**: 이미 다른 사용자에게 연동된 social_id는 409(`SOCIAL_ACCOUNT_TAKEN`)로 막는다 — 허용하면 그 소셜 계정의 주인이 다음 로그인부터 공격자 계정으로 들어온다. 같은 제공자를 두 번 붙이는 것도 막는다(`SOCIAL_ALREADY_LINKED`) — `(user_id, provider)` UNIQUE가 없어 행이 둘 생기면 그때부터 provider 단건 조회가 터진다. 해제는 그 계정에 로그인 수단이 하나도 남지 않게 되면 409(`LAST_LOGIN_METHOD`)로 막는다 — 통과시키면 다시는 들어갈 수 없고 탈퇴조차 못 해 데이터만 남는다. 비밀번호 없는 LOCAL 행은 로그인 수단으로 세지 않는다.

**닉네임 충돌 처리**: `users.nickname`은 UNIQUE다(V5). 소셜이 가입 경로가 아니게 되면서 이 문제가 남는 자리는 **탈퇴 후 유예기간 내 재활성화**뿐이다 — 그때 제공자가 준 닉네임이 그 사이 남에게 쓰이고 있을 수 있다. 여기서도 **중복을 거절할 수 없다**: 닉네임은 제공자가 주는 값이고 사용자가 고른 적이 없어, 흔한 이름이라는 이유로 예외를 던지면 그 사람은 복귀 자체를 못 한다. 그래서 소셜 경로는 `NicknameAllocator`가 번호를 붙이고(`홍길동` → `홍길동2`), 사용자가 직접 입력하는 자체 가입만 409(`NICKNAME_ALREADY_REGISTERED`)로 거절한다. 근거는 [D-028](DecisionLog.md#d-028-유일성-규칙--닉네임은-중복-불가-식당은-카카오-place-id-기준으로-하나).
...
그 외 API 인증 실패(401) 시 프론트는 로그인 화면으로 리다이렉트, 로그인 후 returnUrl로 복귀 — 2026-08-15 구현했다. 복원 경로 검증을 포함한 동작은 [Specification.md 6](Specification.md#6-uiux-화면-설계) 참고.
...
**스케줄러 실행 로그**: `WithdrawnUserCleanupScheduler`는 회차마다 시작·완료 두 줄을 **대상이 0명이어도** 남긴다.

- 조용한 날이 "돌았는데 지울 게 없었다"인지 "아예 안 돌았다"인지 구분되지 않으면 관측이 아니다. 안 도는 경우는 실제로 생긴다 — 04:00에 컨테이너가 떠 있지 않았거나, 재배포가 겹쳤거나, cron/timezone이 어긋났거나. 그 상태는 유예 30일이 지난 개인정보가 계속 남아 있다는 뜻이다.
- 완료 줄에 대상·성공·실패 수와 소요 시간을 적고, **실패가 하나라도 있으면 WARN**으로 올린다. INFO로 두면 정상 회차와 섞여 묻힌다.
- 삭제한 `userId`를 줄마다 남긴다 — [7.5의 오삭제 복구 절차](#75-백업-및-복구-정책)가 이 줄에 의존한다. 행이 이미 없어 DB로는 "언제 지워졌나"를 확인할 수 없다.
- 회차마다 MDC에 상관관계 식별자를 심어 유저별 실패 줄과 요약 줄이 같은 traceId로 묶인다(요청 경로와 같은 형식).
