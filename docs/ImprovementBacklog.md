...
| 네이버 지도 API 구 도메인 | #8 · #76 | `naveropenapi.apigw.ntruss.com`이 신규 도메인(`maps.apigw.ntruss.com`)으로 이전 공지된 이력이 있다. 다만 **공식 API 문서는 2026-08 기준 여전히 구 도메인을 기재**하고 있어 코드에서 바꾸지 않았다. 대신 두 URL을 `NAVER_MAPS_GEOCODE_URL` / `NAVER_MAPS_REVERSE_GEOCODE_URL` env로 노출해 **재빌드 없이 바꾸고 되돌릴 수 있게** 했다. 운영 로그에 `네이버 Geocode API 응답 오류: status=401`(또는 404)이 보이면 이전이 실제로 일어난 것이므로 env만 바꿔 재기동한다 |
