...
> 최신 구현·배포·운영 상태 요약은 [CurrentStatus.md](CurrentStatus.md)를 먼저 본다.
...
> **해소**: [Planning.md 7.5 백업 및 복구 정책](Planning.md#75-백업-및-복구-정책) 신설. 백업 주기·보존기간·RPO/RTO 목표치와, 하드삭제 배치가 남기는 `userId` 로그를 활용한 오삭제 복구 절차를 정의했다. 실제 백업 도구 적용은 운영 DB 확정(8번 CI/CD와 연동) 이후 후속 작업.
...
### 12. 성능 목표 검증·모니터링 최소선 ✅ (2026-08-05 최소선 구축, 2026-08-14 운영 노출 경로 확보, 2026-09-04 1차 회차 실행)

> **2026-09-04 실행**: 부하 테스트를 **처음 실제로 돌렸다.** 결과는 [LoadTestResults.md](LoadTestResults.md)에 있다.
> 요약하면 목표(동시 100명)는 여유롭게 통과하고(픽 P95 61ms), **무릎은 450~500 rps**,
> **1차 병목은 DB 커넥션 풀 10개**다(무릎에서 active 10/10 · pending 40 = 톰캣 스레드 50개 전량 소진,
> 힙은 347MB/3072MB로 한가). 아래 "부하 테스트 실행 자체는 이번 세션에서 실행하지 않았다"는
> 보류가 이로써 해소됐다.
>
> 회차 중 결함 두 개를 찾았다. **(1)** `scripts/k6/seed.sql`이 V7 이전 콜레이션
> (`utf8mb4_unicode_ci`)에 고정돼 있어 실행하면 즉시 죽었다 — 시드가 그동안 한 번도
> 실행된 적이 없었다는 뜻이다. **(2)** nginx 컨테이너의 `ulimit -n`이 1024인데 워커 4개 ×
> `worker_connections` 1024 = 4,096을 쓰려 해서, 무릎 부근에서 `socket() failed (24: No file
> descriptors available)`로 500을 1,583건 냈다 — **앱 로그에는 그 500을 낼 예외가 하나도 없어**
> 백엔드 장애로 오인하기 쉬운 형태였다. 둘 다 고쳤다.
>
> **남은 것**: `heavy` 프로파일(메뉴 수 민감도)과 `soak`은 아직 미실행. 커넥션 풀 상향은
> MySQL `max_connections`와 함께 정해야 할 용량 결정이라 별도로 다룬다.
...
## 2026-08-15 프론트 접근성·복귀 경로·CI 배치

규모와 무관하게 작은 항목들만 모아 처리했다. 셋 다 **화면을 봐서는 멀쩡해 보이는** 결함이라, 눈으로 확인하는 방식으로는 회귀를 잡을 수 없어 테스트로 고정했다.

| 항목 | 무엇이 문제였나 | 처리 |
| --- | --- | --- |
| 로그인 후 복귀 | `ProtectedRoute`가 목적지를 남기지 않아 로그인 후 늘 `/menus`로 떨어졌다. 히스토리를 보다 세션이 끊긴 사용자는 로그인하고 다시 찾아 들어가야 했다 | `auth/returnTo.ts`. sessionStorage에 남기고 이메일·소셜 양쪽에서 복귀. 꺼낼 때 앱 내부 경로인지 검증한다 — 저장소는 같은 오리진 스크립트면 누구나 쓸 수 있어 담긴 값을 우리가 넣었다고 가정할 수 없다 |
| `h1` 계층 | 앱 안쪽 화면이 전부 `h2`로 시작해 `h1`이 하나도 없었다. 제목 목록만 훑는 스크린리더 사용자에게는 화면 이름이 아예 없는 문서였다 | 화면 제목을 `h1`, 폼 제목을 `h2`로 올리고 크기는 `.page h1`로 맞췄다. 로그인 화면의 56px `h1`(서비스 이름)과 역할이 달라 CSS를 나눴다 |
| `aria-pressed` | 카테고리·거리·태그·기간 버튼이 선택 상태를 색으로만 알렸다. 색은 보조기술로 전달되지 않아 필터 화면 전체가 상태 없는 버튼 무더기로 읽혔다 | `a11y/chipToggle.ts`가 클래스와 `aria-pressed`를 함께 만든다. 따로 쓰면 클래스만 고치고 aria를 빠뜨려도 **화면상 아무 이상이 없어** 발견되지 않는다 |
| 폼 초점 관리 | 메뉴·식당의 "수정"은 카드를 통째로 폼으로 갈아끼운다 — 방금 누른 버튼이 사라지면서 초점이 `<body>`로 떨어졌다. 그 뒤 Tab은 폼이 아니라 페이지 맨 위로 갔고, 폼이 열린 사실도 읽히지 않았다 | 열 때 폼 제목으로 초점을 옮기고(`a11y/useFocusOnMount.ts`), 닫을 때 되돌린다. 닫기 전에 잡아 둔 DOM 참조는 쓸 수 없어(그 버튼은 사라졌다) 키로 기억했다가 다시 그려진 버튼을 찾는다 |
| CI에 lint 미실행 | `npm run lint` 스크립트는 있는데 CI가 부르지 않아, 훅 의존성 누락 같은 tsc가 못 잡는 결함이 그대로 통과했다 | frontend 잡에 Lint 단계 추가. 경고로는 실패시키지 않는다 — 기본 경고에 스타일 취향이 섞여 있어 `--deny-warnings`를 켜면 지금 있는 경고 하나 때문에 무관한 PR이 전부 빨개진다 |

---

## 2026-08-15 유일성 규칙 확정 (제품 결정)

닉네임과 식당 중복은 "코드가 정하지 못하는" 항목이라 결정을 기다리고 있었다. 결정 내용과 근거는 [DecisionLog D-028](DecisionLog.md#d-028-유일성-규칙--닉네임은-중복-불가-식당은-카카오-place-id-기준으로-하나).

| 항목 | 결정 | 구현에서 걸린 것 |
| --- | --- | --- |
| 닉네임 중복 | 비허용 | 소셜 로그인은 거절할 수 없다 — 닉네임이 제공자가 주는 값이라, 흔한 이름이면 로그인 자체가 막힌다. 프로필 동의를 거부한 사용자는 전원 같은 기본값을 받아 **두 번째 사용자부터** 충돌한다. 소셜은 번호를 붙이고(`NicknameAllocator`) 자체 가입만 409로 거절한다 |
| 식당 중복 | 카카오 place id 기준, 중복이면 합침 | 이름은 판정에 쓰지 않는다 — 상호가 같은 다른 가게가 실제로 있다. soft delete된 행도 유니크 인덱스 자리를 차지해, 저장 경로가 "없으면 INSERT"가 아니라 "지워진 게 있으면 되살린다"로 동작해야 했다 |

부수적으로 `naver_place_id` 컬럼을 `kakao_place_id`로 고쳤다. 담기는 값은 처음부터 카카오 장소 id였고(장소 검색은 카카오만 쓴다), 유일성 제약이 붙는 컬럼의 이름이 틀린 채로 남으면 나중에 네이버 id를 같은 칸에 섞어 넣게 된다.

**남은 데이터 정리 작업**: 마이그레이션은 기존 중복 행을 합치지 않는다. `naver_url`에 남은 place_url에서 id를 백필하되, 같은 장소가 두 번 저장돼 있으면 먼저 저장한 행만 id를 유지하고 나머지는 NULL로 되돌린다. 실제로 합치려면 `menu_restaurants`·`histories`의 참조를 옮겨야 하는데 `UNIQUE (menu_id, restaurant_id)` 때문에 같은 메뉴가 양쪽에 붙어 있으면 충돌한다 — 스키마 변경이 아니라 데이터 정리라 분리했다. 운영 데이터가 생긴 뒤 NULL로 되돌아간 행이 실제로 있으면 그때 처리한다.

---

## 2026-08-16 SMTP 실발송 검증

공개를 막는 최대 미검증 경로였다. **가입의 모든 경로가 인증 메일을 통과해야 하므로, 발송이 안 되면 아무도 계정을 만들 수 없다.** 그런데 발송 실패는 `AuthMailer`가 삼켜서 가입 응답은 그대로 201이다 — 운영에서 이 결함은 "가입은 되는데 아무도 로그인을 못 하는" 모습으로만 드러난다.

기존 테스트로는 잡을 수 없었다. `EmailSenderTest`·`AuthMailerTest`는 `JavaMailSender`를 목으로 바꿔 "호출했는가"만 보고, 로컬 프로파일은 SMTP를 지정하지 않아 링크를 로그로 떨어뜨린다. 그 경로는 `JavaMailSender` 빈을 만들지도 않으므로 **메일 조립·전송이라는 동작 자체가 한 번도 일어난 적이 없었다.**

| 항목 | 내용 |
| --- | --- |
| `SmtpDeliveryIntegrationTest` | Testcontainers로 Mailpit을 띄우고 **실제 SMTP 소켓**으로 가입 메일이 나가는지 확인한다. 메일에서 토큰을 뽑아 인증까지 통과시키고, 발급된 Access Token으로 `/auth/me`가 되는지까지 본다. 재설정 메일도 링크 착지점(`/reset-password`)이 다른지 함께 고정 |
| 한글 인코딩 | 제목·본문의 한글이 그대로 살아 돌아오는지 단언한다. 깨지면 실제 메일함에서 알아볼 수 없는 메일이 되는데, 로그 폴백 경로로는 절대 드러나지 않는다 |
| `docker-compose.yml`의 mailpit | 로컬에서 실제 발송을 눈으로 보기 위한 개발용 메일 서버(SMTP 1025, UI 8025). 인증이 없어 루프백만 바인딩한다 |

**여전히 덮지 못하는 것**: 운영 설정은 `starttls.enable=true` + `auth=true`인데 테스트는 평문 Mailpit을 상대한다. **TLS 핸드셰이크와 SMTP 인증은 실제 제공자(Gmail/SES 등)를 붙일 때 따로 확인해야 한다.** 이번에 닫은 구멍은 "SMTP 대화가 성립하고 메일이 제대로 조립돼 나가는가"까지다.
