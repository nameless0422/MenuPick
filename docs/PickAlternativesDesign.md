...
**상태: 구현 완료·기본 비활성·미배포 / 기준일: 2026-09-14**

이 문서는 수동 픽에서 조건 때문에 후보가 0개일 때, 사용자가 명시적으로 선택할 수 있는
최소 범위의 조정안을 제시하는 계약이다. 백엔드 API와 프론트 화면 및 대상 테스트는 구현됐지만
기능 플래그 기본값은 `false`이고, 이 문서 작성 시점에 PR·배포·롤아웃은 완료되지 않았다.
...
blocker 0건을 확인했다. 아래 기능 매트릭스의 구현 검증은 완료됐으며 배포 검증만 PR·릴리스
이후 남아 있다.
...
현재 롤아웃은 시작하지 않았고 성공 지표 베이스라인도 아직 없다.
