    ...
    <div className="login-page">
      ...
      <section className="card login-demo">
        ...
        <p className="login-demo-desc">
          <strong>{email}</strong> 으로 메일을 보내고 있어요. 링크는 24시간 동안 유효합니다.
        </p>
        {/* 서버는 이미 가입된 주소와 새 주소를 같은 응답으로 돌려준다 — 응답만 보고 가입
            여부를 캐낼 수 없게 하기 위해서다. 그래서 이 화면도 "인증 링크를 보냈다"고
            단언할 수 없다. 두 경우에 실제로 무엇이 오는지를 함께 적어야, 인증 메일 대신
            로그인 안내를 받은 사람이 링크를 기다리며 막히지 않는다. */}
        <p className="login-demo-desc">
          처음 가입하는 주소라면 인증 링크가, 이미 가입된 주소라면 로그인 안내가 갑니다.
        </p>
